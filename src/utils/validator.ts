...
export function parseThreshold(threshold: string): ParsedThreshold {
	...
	if (colonIndex === -1) {
		throw new Error(
			`Invalid threshold format: '${threshold}'. Expected 'metric:condition' (e.g., 'http_req_duration:p(95)<500')`,
		);
	}
	...
	if (!metric || !condition) {
		throw new Error(
			`Invalid threshold format: '${threshold}'. Both metric and condition must be non-empty.`,
		);
	}
...
export function validateMethod(method: string): void {
	if (!(ALLOWED_METHODS as readonly string[]).includes(method.toUpperCase())) {
		throw new Error(
			`Invalid HTTP method: ${method}. Allowed methods: ${ALLOWED_METHODS.join(", ")}`,
		);
	}
}
...
export function validateProxy(proxyUrl: string): void {
	...
	try {
		...
		throw new Error(
			`Invalid proxy URL: ${proxyUrl}. Expected format: http://host:port or https://host:port or socks5://host:port`,
		);
	}
	...
	if (!validProtocols.includes(parsed.protocol)) {
		throw new Error(
			`Unsupported proxy protocol: ${parsed.protocol}. Use http, https, socks5, or socks5h.`,
		);
	}
	...
	if (!parsed.port) {
		throw new Error(
			`Proxy URL must include a port. Example: ${parsed.protocol}//${parsed.hostname}:8080`,
		);
	}
}
