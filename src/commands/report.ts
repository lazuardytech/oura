	...
	.action(async (options) => {
		try {
			...
			if (httpReqs) {
				...
				console.log(
					`Req Rate       : ${(httpReqs.values?.rate || 0).toFixed(2)}/s`,
				);
			}
			if (httpReqDuration) {
				console.log(
					`Avg Duration   : ${(httpReqDuration.values?.avg || 0).toFixed(2)}ms`,
				);
				console.log(
					`P95 Duration    : ${(httpReqDuration.values?.["p(95)"] || 0).toFixed(2)}ms`,
				);
				console.log(
					`P99 Duration    : ${(httpReqDuration.values?.["p(99)"] || 0).toFixed(2)}ms`,
				);
				console.log(
					`Max Duration   : ${(httpReqDuration.values?.max || 0).toFixed(2)}ms`,
				);
			}
			if (httpReqFailed) {
